...
export function Dashboard({ goalsList, formatCurrency, onSelectGoal }: DashboardProps) {
  ...
  const upcomingPayments = useMemo(() => {
    ...
    return goalsList
      .filter((g) => {
        const totalG = g.totalValue * (1 + (g.interestRate || 0) / 100);
  ...
  const StatCard = ({ icon, label, value, sub, color }: any) => (
    <div className="glass-card-subtle p-4 flex flex-col gap-2">
        ...
        {upcomingPayments.length === 0 ? (
          ...
          <div className="space-y-3">
            {upcomingPayments.map(({ goal, nextDue }) => {
              ...
              const totalG = goal.totalValue * (1 + (goal.interestRate || 0) / 100);
